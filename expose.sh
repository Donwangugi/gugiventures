...
download_file="download.txt" # search for this file in each gallery directory for photos to download and their captions
...
do
  ...
  if [[ "$node" =~ "/.git" ]] #do not want to scan git files
  then
    continue
  fi

  #check if a download file exists
  if [[ -f "$node/$download_file" ]]
  then
    echo "It worked\n"
    #cat "$node/$download_file" | while read LINE
    #do
    #  let count++
    #  echo "$LINE"
    #  filename=$(basename "$LINE" | sed 's/.*\/\([^\/]*\..*\)$/\1/g')
    #  echo "$filename"
    #  curl -# -C - -o "${2}" "${1}"
    cat "$node/$download_file" | while read CMD
    do
      echo "$CMD"
      filename=$(basename "$CMD" | sed 's/.*\/\([^\/]*\..*\)$/\1/g')
      echo "$node/$filename"
      curl -# -C - -o "$node/$filename" "$CMD"
    done
  fi
  echo "$node"
...
do
	...
	do
		...
		# if perl available, pass content through markdown parser #TODO - THIS USES MARKDOWN, TRY THAT LATER
